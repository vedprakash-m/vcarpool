...
class BackupAndRecoveryManager {
    ...
    async performFullBackup() {
        ...
        try {
            ...
            logger_1.logger.info('Full backup completed', { backupId, size: metadata.size, collections: metadata.collections.length });
            return metadata;
        }
        catch (error) {
    ...
    async restoreFromBackup(options) {
        ...
        try {
            ...
            if (options.validateOnly) {
                ...
                logger_1.logger.info('Backup validation completed', { backupId: options.backupId });
                return;
            }
            ...
            logger_1.logger.error('Restore operation failed', { options, error });
            throw error;
        }
    }
    /**
    ...
    async executeDisasterRecovery() {
        logger_1.logger.warn('Executing disaster recovery plan');
        try {
            // Send notifications
            await this.notifyDisasterRecoveryTeam();
            // Execute recovery steps in order
            for (const step of this.recoveryPlan.recoverySteps) {
                logger_1.logger.info(`Executing recovery step: ${step.description}`, { stepId: step.id });
    ...
    async cleanupOldBackups() {
        ...
        for (const backup of backupsToDelete) {
            try {
                ...
                logger_1.logger.warn('Failed to delete old backup', { backupId: backup.id, error });
            }
        }
    }
    ...
    async executeRecoveryScript(script) {
        try {
            ...
            logger_1.logger.info('Recovery script executed successfully', { script, stdout, stderr });
        }
        catch (error) {
