...
class ValidationMiddleware {
     ...
     */
    static secureString = (maxLength = 255) => zod_1.z.string()
        .trim()
    ...
    static secureEmail = zod_1.z.string()
        .email('Invalid email format')
        .max(254, 'Email too long')
        .toLowerCase()
        .refine((val) => this.isValidEmailDomain(val), 'Email domain not allowed');
    static secureUrl = zod_1.z.string()
        .url('Invalid URL format')
        .refine((val) => this.isAllowedUrl(val), 'URL not allowed');
    static secureId = zod_1.z.string()
        .uuid('Invalid ID format');
    static securePhoneNumber = zod_1.z.string()
        .regex(/^\+?[\d\s\-\(\)]{10,15}$/, 'Invalid phone number format')
        .transform((val) => val.replace(/\s/g, ''));
    static securePassword = zod_1.z.string()
...
//# sourceMappingURL=enhanced-validation.middleware.js.map