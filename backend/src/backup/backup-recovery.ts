...
class BackupAndRecoveryManager {
  ...
  async performFullBackup(): Promise<BackupMetadata> {
    ...
    try {
      ...
      logger.info('Full backup completed', { backupId, size: metadata.size, collections: metadata.collections.length });
  ...
  async restoreFromBackup(options: RestoreOptions): Promise<void> {
    ...
    try {
      ...
      if (options.validateOnly) {
        ...
        logger.info('Backup validation completed', { backupId: options.backupId });
        return;
      }
      ...
      logger.error('Restore operation failed', { options, error });
      throw error;
    }
  }
  ...
  async executeDisasterRecovery(): Promise<void> {
    ...
    try {
      ...
      for (const step of this.recoveryPlan.recoverySteps) {
        logger.info(`Executing recovery step: ${step.description}`, { stepId: step.id });

        if (step.automated && step.script) {
          await this.executeRecoveryScript(step.script);
        } else {
          logger.warn(`Manual intervention required for step: ${step.description}`, { stepId: step.id });
        }
      }
  ...
  getBackupMetrics(): {
    ...
    return {
      ...
      lastFullBackup: fullBackups.length > 0 ? 
        new Date(Math.max(...fullBackups.map(b => b.timestamp.getTime()))) : undefined,
      lastIncrementalBackup: incrementalBackups.length > 0 ? 
        new Date(Math.max(...incrementalBackups.map(b => b.timestamp.getTime()))) : undefined,
  ...
  private async backupCollection(databaseId: string, collectionId: string, backupId: string): Promise<void> {
  ...
  private async cleanupOldBackups(): Promise<void> {
    ...
    for (const backup of backupsToDelete) {
      try {
        ...
        logger.warn('Failed to delete old backup', { backupId: backup.id, error });
      }
    }
  }
  ...
  private async executeRecoveryScript(script: string): Promise<void> {
    try {
      ...
      logger.info('Recovery script executed successfully', { script, stdout, stderr });
    } catch (error) {
  ...
  private getCollectionBackupPath(backupId: string, databaseId: string, collectionId: string): string {
