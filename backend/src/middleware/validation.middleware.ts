...
export function validateQueryParams<T>(schema: ZodSchema<T>) {
  return function(
    handler: (request: HttpRequest & { validatedQuery: T }, context: any) => Promise<HttpResponseInit>
  ) {
 ...
 */
export function validatePathParams<T>(schema: ZodSchema<T>, paramExtractor: (request: HttpRequest) => Record<string, string>) {
  return function(
    handler: (request: HttpRequest & { validatedParams: T }, context: any) => Promise<HttpResponseInit>
  ) {
    return async (request: HttpRequest, context: any): Promise<HttpResponseInit> => {
      try {
